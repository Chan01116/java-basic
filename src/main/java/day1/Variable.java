package day1;

public class Variable {
    public static void main(String[] args) {

        //홀길동의 나이가 20.
        //홍길동 나이가 21로 올라감
        System.out.println(20);
        System.out.println(20);
        System.out.println(20);
        System.out.println(20);
        System.out.println(20);

        // 변수를 사용하여 값을 저장할 수 있다.

        // 변수는 상자
        // 상자는 이름이 있어야 함.
        // 상자는 크기와 모양이 있어야 함.

        // 숫자
        // 상자(변수)를 만들어야 함
        //int(숫자) age(이름);

        // 값을 변수에 담아야함,
        // =을 이용해서 저장가능
        int age;
        age = 20;
        int age2;
        age2 = 21;


        // 문자
        //String(문자) name(이름);
        String name;
        String name2;
        name = "홍길동";
        name2 = "홍길만";

                //변수의 값을 꺼내서 사용
        System.out.println(age);
        System.out.println(name);
         System.out.println("안녕하세요 저는 " + (age) + "살 " + (name) + " 입니다 ");
        System.out.println("안녕하세요 저는 " + (age) + "살 " + (name) + " 입니다 ");
        System.out.println("안녕하세요 저는 " + (age) + "살 " + (name) + " 입니다 ");
        System.out.println("안녕하세요 저는 " + (age) + "살 " + (name) + " 입니다 ");
        System.out.println("안녕하세요 저는 " + (age) + "살 " + (name) + " 입니다 ");
        System.out.println("안녕하세요 저는 " + (age) + "살 " + (name) + " 입니다 ");
        System.out.println("안녕하세요 저는 " + (age) + "살 " + (name) + " 입니다 ");
        System.out.println("안녕하세요 저는 " + (age)+ "살 " + (name) + " 입니다 ");
        System.out.println("안녕하세요 저는 " + (age) + "살 " + (name) + " 입니다 ");
        System.out.println("안녕하세요 저는 " + (age) + "살 " + (name) + " 입니다 ");
        System.out.println("안녕하세요 저는 " + (age) + "살 " + (name) + " 입니다 ");
        System.out.println("안녕하세요 저는 " + (age) + "살 " + (name) + " 입니다 ");



        System.out.println("안녕하세요 저는 " + (age2) + "살 " + (name2) + " 입니다 ");
        System.out.println("안녕하세요 저는 " + (age2) + "살 " + (name2) + " 입니다 ");
        System.out.println("안녕하세요 저는 " + (age2) + "살 " + (name2) + " 입니다 ");
        System.out.println("안녕하세요 저는 " + (age2) + "살 " + (name2) + " 입니다 ");
        System.out.println("안녕하세요 저는 " + (age2) + "살 " + (name2) + " 입니다 ");
        System.out.println("안녕하세요 저는 " + (age2) + "살 " + (name2) + " 입니다 ");
        System.out.println("안녕하세요 저는 " + (age2) + "살 " + (name2) + " 입니다 ");
        System.out.println("안녕하세요 저는 " + (age2)+ "살 " + (name2) + " 입니다 ");
        System.out.println("안녕하세요 저는 " + (age2) + "살 " + (name2) + " 입니다 ");
        System.out.println("안녕하세요 저는 " + (age2) + "살 " + (name2) + " 입니다 ");
        System.out.println("안녕하세요 저는 " + (age2) + "살 " + (name2) + " 입니다 ");
        System.out.println("안녕하세요 저는 " + (age2) + "살 " + (name2) + " 입니다 ");

        // 안녕하세요 저는 20살 홍길동입니다. 10번 출력

        // 홍길도의 나이가 21살이 되었습니다. 21살로 바꿔서 10번 출력해주세여

        //홍길동이 홍길만으로 개명을 했습니다. 바꿘 이름을 적용해서 10번 출력해주시요\
        // 해설 System.out.println("안녕하세요 저는" + age2 + "살 "+ name2 +"입니다.");

        // 변수 사용시 주의사항
        // 동일한 이름의 변수가 2개 이상 존재할 수 없다.
        // 변수명이 숫자나 특수기호로 시작해서는 안된다.
        // 변수는 재활용 가능
        // 변수는 = 기준 왼쪽, 저장할 값은 오른쪽


    }
}
